...
+ (NSArray *)defaultPresetRatios {
    return @[
        //        @{
        //            kCLClippingToolRatioValue1 : @0,
        //            kCLClippingToolRatioValue2 : @0,
        //            kCLClippingToolRatioTitleFormat :
        //                [CLImageEditorTheme localizedString:@"CLClippingTool_ItemMenuCustom" withDefault:@"Custom"]
        //        },
        @{
...
- (void)setup {
    ...
    if (!swapBtnHidden) {
        ...
        [btn setImage:[self imageForKey:kCLClippingToolRotateIconName defaultImageName:@"btn_rotate.png"]
...
- (CLClippingCircle *)clippingCircleWithTag:(NSInteger)tag {
    ...
    UIPanGestureRecognizer *panGesture =
        [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(panCircleView:)];
...
- (id)initWithSuperview:(UIView *)superview frame:(CGRect)frame {
    ...
    if (self) {
        ...
        UIPanGestureRecognizer *panGesture =
            [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(panGridView:)];
...
- (void)setClippingRect:(CGRect)clippingRect {
    ...
    _ltView.center =
        [self.superview convertPoint:CGPointMake(_clippingRect.origin.x, _clippingRect.origin.y) fromView:self];
    _lbView.center = [self.superview convertPoint:CGPointMake(_clippingRect.origin.x, _clippingRect.origin.y + _clippingRect.size.height)
                                         fromView:self];
    _rtView.center = [self.superview convertPoint:CGPointMake(_clippingRect.origin.x + _clippingRect.size.width, _clippingRect.origin.y)
                                         fromView:self];
    _rbView.center = [self.superview convertPoint:CGPointMake(_clippingRect.origin.x + _clippingRect.size.width, _clippingRect.origin.y + _clippingRect.size.height)
                                         fromView:self];
...
- (void)setClippingRect:(CGRect)clippingRect animated:(BOOL)animated {
    if (animated) {
        [UIView animateWithDuration:kCLImageToolFadeoutDuration
                         animations:^{
                           _ltView.center = [self.superview convertPoint:CGPointMake(clippingRect.origin.x, clippingRect.origin.y)
                                                                fromView:self];
                           _lbView.center = [self.superview convertPoint:CGPointMake(clippingRect.origin.x, clippingRect.origin.y + clippingRect.size.height)
                                                                fromView:self];
                           _rtView.center = [self.superview convertPoint:CGPointMake(clippingRect.origin.x + clippingRect.size.width, clippingRect.origin.y)
                                                                fromView:self];
                           _rbView.center = [self.superview convertPoint:CGPointMake(clippingRect.origin.x + clippingRect.size.width, clippingRect.origin.y + clippingRect.size.height)
                                                                fromView:self];
                         }];
